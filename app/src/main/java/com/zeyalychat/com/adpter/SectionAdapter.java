...
public class SectionAdapter extends RecyclerView.Adapter<SectionAdapter.ViewHolder> {
    ...
    public SectionAdapter(Activity mContext, ArrayList<SectionBean> myDataset) {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
        if (mDataset.get(position).getSection_visibility()) {
            binding.textDate.setText(mDataset.get(position).getGrade_name()+" "+mDataset.get(position).getSection_name());
        }else {
